...
/**
 * Residual pill shown while the overlay map is auto-hidden on the current
 * map. Click = temporarily show the overlay until the player leaves the map
 * (session-only; the persisted flag is untouched).
...
export function OverlayMapHiddenPill() {
  const { hidden, playerMap, toggleOverride } = useOverlayMapHidden();
  const lockedWindow = useSettingsStore((s) => s.lockedWindow);
  const t = useT();
  if (!hidden || !playerMap || lockedWindow) return null;
  return (
