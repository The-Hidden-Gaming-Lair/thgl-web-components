...
export function StreamingReceiver({
  ...
  const { setLiveMode, liveMode, appId, setAppId } = useSettingsStore(
    useShallow((state) => ({
      ...
      <DialogContent className="sm:max-w-[425px]">
        <DialogHeader>
          ...
          <DialogDescription>
            ...
            to sync your player position
            {withoutLiveMode ? "" : " and spawned nodes"} with this map on every
                ...
                {!withoutLiveMode && (
                  <Label
                    className={cn("flex items-center gap-2 transition-colors", {
                      "hover:text-primary cursor-pointer": connection,
                    })}
                  >
                    <Switch
                      checked={liveMode}
