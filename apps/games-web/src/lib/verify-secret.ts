 ...
 * enriched secret.
 *
 * Served under two route names by thin wrappers:
 *  - /api/patreon/verify   — app-agnostic; used by the THGLApp/web session
 *    self-heal (reverifyAccountSecret in @repo/lib).
 *  - /api/patreon/overwolf — legacy name kept for shipped Overwolf apps.
 */
export async function verifySecretPOST(request: NextRequest) {
  try {
    ...
    try {
      ...
      console.error(`[patreon/verify] getToken failed for ${userId}: ${msg}`);
    }
    ...
    if (storeUnavailable && !storedToken && embeddedToken) {
      ...
      // (and no way to update the secret the client holds) the rotated
      ...
      if (
        ...
        console.warn(
          `[patreon/verify] stored refresh token rejected (${refreshTokenResponse.status}) for ${userId} — retrying with embedded token`,
        );
    ...
    const result = {
      ...
      // it back so the secret self-updates like the web cookie; older
      // versions simply ignore the field.
