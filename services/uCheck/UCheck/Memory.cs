...
public class Memory(ProcessSummary proc)
{
    [DllImport("user32")]
    ...
    public ProcessSummary Process { get; } = proc;

    public nint BaseAddress => Process.MainModuleBase;


    public int MaxStringLength = 0x100;


    public byte[] ReadProcessMemory(nint addr, int length)
    {
        return Mem.Instance.ReadProcessBytes(Process.ProcessId, addr, length);
    }
    ...
    public object ReadProcessMemory(Type type, nint addr)
    {
        ...
        var buffer = Mem.Instance.ReadProcessBytes(Process.ProcessId, addr, Marshal.SizeOf(type));
    ...
    public nint FindPattern(string pattern, nint start, ulong length)
    {
