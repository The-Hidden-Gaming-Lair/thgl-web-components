        ...
        for (var l = 0; l < levels.Num; l++)
        {
          ...
          for (var i = 0; i < actorsNum; i++)
          {
            try
            {
              var actor = Actors[i];
              string type = null;
              var npcData = actor["NPCData"];
              if (npcData != null)
              {
                type = npcData.ClassName.Split('/').Last().Split('.').First();
              } else {
                var instancesStaticMeshComponent = actor["InstancedStaticMeshComponent"];
                var staticMesh = instancesStaticMeshComponent?["StaticMesh"];
                if (staticMesh != null)
                {
                  type = staticMesh.ClassName.Split('/').Last().Split('.').First();
                  if (type == "iron_deposit" && mapName == "gallia_pve_01")
                  {
                    type = "iron_deposit_pure";
                  }
                }
              }
              if (type == null) { 
