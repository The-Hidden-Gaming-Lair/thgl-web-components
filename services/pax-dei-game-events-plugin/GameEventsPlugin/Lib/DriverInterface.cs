...
namespace GameEventsPlugin
{
  ...
  public class DriverInterface
  {
    ...
    private readonly IntPtr _driverHandle = WinApi.INVALID_HANDLE_VALUE;

    public DriverInterface(string registryPath)
    {
      _registryPath = registryPath;
      _driverHandle = WinApi.CreateFileA(_registryPath, FileAccess.ReadWrite, FileShare.ReadWrite, IntPtr.Zero, FileMode.Open, 0,
    IntPtr.Zero);
    }
    ...
    public bool GetProcessSummaryList(out ProcessSummary[] result)
    {
      ...
      var reader = new BinaryReader(new MemoryStream(managedBuffer));
