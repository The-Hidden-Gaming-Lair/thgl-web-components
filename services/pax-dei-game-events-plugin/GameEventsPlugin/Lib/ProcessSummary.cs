...
namespace GameEventsPlugin
{
  public class ProcessSummary
  {
    ...
    public IntPtr MainModuleBase { get; set; }
    ...
    public static ProcessSummary ProcessSummaryFromId(DriverInterface driver, int processId)
    {
      if (!driver.GetProcessSummaryList(out var processes)) throw new InvalidOperationException("Can't query processes.");
      var result = processes.FirstOrDefault(p => p.ProcessId == processId);
      if (result == null)
      {
    ...
    {
      ProcessId = processId;
      MainModuleBase = (IntPtr)(long)mainModuleBase;
    ...
    private string FixFileName(string fileName)
    {
      ...
      else
      {
        ...
        var length = WinApi.GetLongPathName(fileName, sb, sb.Capacity);
        var flag2 = length > sb.Capacity;
        if (flag2)
        {
          sb.Capacity = length;
    ...
    {
      return new ProcessSummary(reader.ReadInt32(), reader.ReadUInt64(), Encoding.Unicode.GetString(reader.ReadBytes(512)).Split(new char[1])[0],
          reader.ReadUInt32(), reader.ReadUInt64(), reader.ReadBoolean());
    }
  }
}