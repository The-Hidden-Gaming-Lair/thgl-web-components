...
// Footprint Riddle
{
  ...
  const triggerDataRiddleS04Shadow = await readJSON<TriggerDataRiddleS04Shadow>(
    CONTENT_DIR +
      "/game_common/data/task/trigger_data/trigger_data_riddle_s04_footprint.json",
  );

  const type = "riddle_footprint";
  enDict[type] = "Footprint";
  const iconPath = String.raw`${TEMP_DIR}\game-icons\jigsaw-piece_lorc.png`;
  const iconProps: IconProps = {
    // border: true,
    color: uniqolor(type, {
...
// Cave Riddle
{
  ...
  const triggerDataRiddleS04Shadow = await readJSON<TriggerDataRiddleS04Shadow>(
    CONTENT_DIR +
      "/game_common/data/task/trigger_data/trigger_data_riddle_s04_cave.json",
  );

  const type = "riddle_cave";
  enDict[type] = "Cave";
  const iconPath = String.raw`${TEMP_DIR}\game-icons\jigsaw-piece_lorc.png`;
  const iconProps: IconProps = {
    // border: true,
...
// Balloon Riddle
{
  ...
  const triggerDataRiddleS04Shadow = await readJSON<TriggerDataRiddleS04Shadow>(
    CONTENT_DIR +
      "/game_common/data/task/trigger_data/trigger_data_riddle_s04_balloon.json",
  );

  const type = "riddle_balloon";
  enDict[type] = "Balloon";
  const iconPath = String.raw`${TEMP_DIR}\game-icons\jigsaw-piece_lorc.png`;
  const iconProps: IconProps = {
    // border: true,
  ...
  for (const nodeData of placeNodes) {
    ...
    if (!nodeData.sfx_path?.endsWith("stardust_fluid_mist_end.pse")) {
      continue;
    }
    const id = `${type}_${nodeData.no}`;
    const spawn: Node["spawns"][0] = {
...
for (const fish of Object.values(fishData)) {
  ...
  if (!newTypes.includes(type)) {
    newTypes.push(type);
    const icon = await addToIconSprite(iconPath, type, {});
    if (!filters.some((f) => f.group === group)) {
      filters.push({
        group,
        values: [],
...
// Morphic Crate
{
  ...
  const icon = await addToIconSprite(
