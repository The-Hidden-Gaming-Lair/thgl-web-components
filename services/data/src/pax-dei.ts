...
initDirs(
  ...
  String.raw`C:\dev\PaxDei\Extracted\Texture`,
  String.raw`C:\dev\the-hidden-gaming-lair\static\pax-dei`,
);

let nodes = await readJSON<Node[]>(OUTPUT_DIR + "/coordinates/nodes.json");
...
const icons: Record<string, Icon> = {
  ...
  //   `${TEMP_DIR}\\game-icons\\portal_lorc.pngp`
  // ),
};
...
const mapMarkerTypes = await readJSON<MapMarkerTypes>(
...
const localisationEN = await readJSON<Record<string, string>>(
  CONTENT_DIR +
...
for (const mapName of readDirSync(
  ...
  const domainConfig = await readJSON<DomainConfig>(
    CONTENT_DIR +
  ...
  if (Bun.argv.includes("--tiles")) {
    try {
      ...
      const canvas = await mergeImages(bigTiles, /x(\d+)_y(\d+)/);
  ...
  for (const mapCellFilename of mapCellFilenames) {
    const mapCell = await readJSON<MapCell>(
      CONTENT_DIR +
...
for (const gatherablesPath of gatherablesPaths) {
  const file = await readJSON<GatherablesDataAsset>(gatherablesPath);
  ...
  const pdaResource = await readJSON<PDAResourcesGatherables>(
    CONTENT_DIR +
  ...
  const resource = await readJSON<Resource>(resourcePath);
  let typeId = resource
    .find((r) => r.Outer)!
  ...
  if (!iconFilename) {
    if (id === "fruit_apple_red") {
      ...
      iconPath = `${TEMP_DIR}\\game-icons\\berry-bush_delapouite.png`;
    }
  } else {
...
for (const mineablesPath of mineablesPaths) {
  const file = await readJSON<GatherablesDataAsset>(mineablesPath);
  const id = file[0].Properties.ResourceName;
  ...
  const pdaResource = await readJSON<PDAResourcesMineables>(
    CONTENT_DIR +
  ...
  if (!iconFilename) {
    if (id === "gneiss_deposit") {
      ...
      iconPath = `${TEMP_DIR}\\game-icons\\ore_faithtoken.png`;
    }
  } else {
    iconPath = (resourceFolder + "/" + iconFilename)
      .replace(CONTENT_DIR, "")
...
for (const npcsResourcesPath of readDirRecursive(
  ...
  const npc = await readJSON<NPCDataAssets>(npcsResourcesPath);
  ...
  if (npc[0].Name.includes("_Badger_")) {
    iconPath = `${TEMP_DIR}\\game-icons\\badger_caro-asercion.png`;
  } else if (npc[0].Name.includes("_Bear_")) {
    iconPath = `${TEMP_DIR}\\game-icons\\bear-face_sparker.png`;
  } else if (npc[0].Name.includes("_Boar_")) {
    iconPath = `${TEMP_DIR}\\game-icons\\boar_caro-asercion.png`;
  } else if (npc[0].Name.includes("_Fox_")) {
    iconPath = `${TEMP_DIR}\\game-icons\\fox-head_lorc.png`;
  } else if (npc[0].Name.includes("_Animal_Wolf_")) {
    iconPath = `${TEMP_DIR}\\game-icons\\hound_lorc.png`;
  } else if (npc[0].Name.includes("_Wolf_")) {
    iconPath = `${TEMP_DIR}\\game-icons\\wolf-head_lorc.png`;
  } else if (npc[0].Name.includes("_Rabbit_")) {
    iconPath = `${TEMP_DIR}\\game-icons\\rabbit-head_delapouite.png`;
  } else if (npc[0].Name.includes("_Deer_")) {
    iconPath = `${TEMP_DIR}\\game-icons\\deer_caro-asercion.png`;
  } else if (npc[0].Name.includes("_Zeb_")) {
    iconPath = `${TEMP_DIR}\\game-icons\\cultist_lorc.png`;
  } else if (npc[0].Name.includes("_Inq_")) {
    iconPath = `${TEMP_DIR}\\game-icons\\light-helm_delapouite.png`;
  } else if (category === "Animals") {
    iconPath = `${TEMP_DIR}\\game-icons\\animal-hide_delapouite.png`;
    ...
    iconPath = `${TEMP_DIR}\\game-icons\\daemon-skull_lorc.png`;
  } else if (category === "Named Demons") {
    iconPath = `${TEMP_DIR}\\game-icons\\daemon-skull_lorc.png`;
    iconSettings.color = "orange";
  } else if (category === "Fey") {
    iconPath = `${TEMP_DIR}\\game-icons\\fairy_delapouite.png`;
  } else if (category === "Inquisitors") {
    iconPath = `${TEMP_DIR}\\game-icons\\templar-shield_delapouite.png`;
  } else if (category === "Named Inquisitors") {
    iconPath = `${TEMP_DIR}\\game-icons\\templar-shield_delapouite.png`;
    iconSettings.color = "orange";
  } else if (category === "LostSouls") {
    iconPath = `${TEMP_DIR}\\game-icons\\soul-vessel_delapouite.png`;
    ...
    iconPath = `${TEMP_DIR}\\game-icons\\monster-grasp_lorc.png`;
  } else {
...
//       const level = await readJSON<RootLevel>(
//         CONTENT_DIR +
//           "/PaxDei/Content/_PD/World/" +
//           mapName +
//           "/" +
...
writeJSON(OUTPUT_DIR + "/coordinates/nodes.json", newNodes);
// encodeToFile(OUTPUT_DIR + "/coordinates/cbor/nodes.cbor", newNodes);
...
writeJSON(OUTPUT_DIR + "/coordinates/filters.json", filtersWithNodes);
...
writeJSON(OUTPUT_DIR + "/coordinates/types_id_map.json", sortedTypesIdMap);
